  ...
  Validate.checkKey(key).then(
    ...
    function (value) {
      ...
      ref.get().then((snapshot) => {
        ...
        if (id) {
          sendData = {
            data: data[0].appointments.findAll((guest) => guest.guestID === id),
            cont: data[0].appointments.findAll((guest) => guest.guestID === id)
              .length,
          };
          res.send(sendData);
        }
        ...
        if (apid) {
          sendData = {
            data: data[0].appointments.findAll((guest) => guest._id === apid),
            cont: data[0].appointments.findAll((guest) => guest._id === apid)
              .length,
          };
          res.send(sendData);
        }
